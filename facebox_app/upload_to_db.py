...
def add_media(conn, container, foldername, folder_id, filename, media_type):
    cursor = conn.cursor()
    extension = os.path.splitext(filename)[1]
    storage_key = container + '::' + foldername + '/' + filename
    insert_query = """
...
def upload_to_db(conn, data_dir, container, folder, owner_name="admin", group_name="editors"):
    ...
    for media_file, uri in uris_dict.items():
        ...
        if record is None:
            ...
            file_id = add_media(conn, container, folder, folder_id, media_file, media_type)
            count_inserted += 1
        else:
            file_id = record[0]
            media_type = record[3]
