...
function displayResults(results) {
    ...
    results.forEach(item => {
        ...
        row.addEventListener('dblclick', function(event) {
            window.open('/app/photo-viewer/?file_id='+item.file_id+'&file_name=photo', '_blank');
        });
        ...
        let html = `
            <td>${item.file_id}</td>
            <td><img id="thumbnail_${item.file_id}" src="" alt="thumbnail"></td>
            <td class="file-name"><a href="${item.file_url}" class="hyperlink" target="_blank">${item.file_name}</a><br><br>
                <a href="#" onclick="goToFaces(${item.file_id},'${item.file_name}')">Faces</a></td>
...
function browseFolder() {
    ...
    function createRootElement() {
        const li = document.createElement('li');
        let folder = { folder_id: 1, path: '', name: '[all folders]', path_name: '/' };
        li.textContent = folder.name;
        li.dataset.folderId = folder.folder_id;
